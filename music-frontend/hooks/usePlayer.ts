...
export function usePlayer(): [
  ...
  const setCurrentTime = (time: number) => setState(s => ({ ...s, currentTime: time }));
  const setDuration = (dur: number) => setState(s => ({ ...s, duration: dur }));
  ...
  return [state, { play, pause, setVolume, setCurrentTime, setDuration, setActiveTrack }];
}